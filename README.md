...
- Clean Code를 위해 **재사용성을 고려한 View 분리**와 **의미 /** **유형별**로 나눠 **최소 단위**로 Commit을 했지만, 해커톤 마감시간으로 인해 끝까지 유지하지 못한점
- PM과 Designer가 만든 **5번째 Tab**(클라우드 펀딩 - 챌리지 Tab)을 시간 내에 구현하지 못한점
- Designer가 원하는 디자인을 구현(Icon, 글꼴, 색상, 투명도 등)한다는 것이 어렵다는 점을 알게됨
- AOS 개발자와 소통하면서 Components의 용어가 다르다는 점을 알게됨
- 짧은 시간내에 목표를 이루기 위해서는 각자의 역할을 **책임감**있게 맡아야한다는 점을 알게됨
- 52시간 동안 3시간을 자면서 9번만(화장실, 식사) 자리에서 일어나는 **몰입의 경험**을 통해 개발을 사랑한다는 것을 알게됨
- 제출 6시간전에 **시연 포기선언**을 했는데, 끝까지 포기하지 않고 개발하여 시연을 할 수 있는 끈기를 알게됨
<br/>
